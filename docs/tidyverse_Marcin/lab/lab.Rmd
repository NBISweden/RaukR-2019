...
## Reading data
Now, let's read the fastq data. Check `ShortRead` documentation to see how to read our `fastq` file. Also, try to use package `here`. If you write: `data <- here::here('data/my.fastq')`, the `my.fastq` file will be read from the `data` folder which is a subfolder of your project root, i.e. the folder where your r script is. It is a good practise and also prevents [Jenny Brian from coming to your office and setting your computer on fire](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/).
