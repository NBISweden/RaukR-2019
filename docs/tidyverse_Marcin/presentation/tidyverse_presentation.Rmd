...
keywords: r, RaukR, markdown, tidyverse
output: 
...
```
--
```{r, fig.width=12, fig.height=4, fig.align='center', echo=F}
 my_rnorms %>% 
...
result$call_result %>% setNames(result$f) %>% as_tibble() %>% gather(result$f, key = 'fun', value = 'num') %>% ggplot(mapping=aes(x=num)) + geom_density() + facet_wrap(~ fun) + theme_bw()
```
