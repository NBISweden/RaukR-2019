...
$\hspace{2cm}$Still not quite right...
...
* By transforming $y$, we get a linear expression in the r.h.s.  $$\log\frac{y}{1+y} = \beta_0+ \beta_1 x$$
]
...
Generate $y = \beta_1* x + \beta_0$, $\beta_0=0.3$ and $\beta_1=0.2$
...
* Probability fo finding a more extreme value or test statistic under the NULL model.
    + p-values can refer to different meanings of *extreme*. Using our model as an example:
