...
* Generate 100 samples of $x,y$ by
  ...
  - generate $y$ for each sample of using the  model $y = \beta_0 + \beta_1* x$ 
  - plot $y$ against $x$.
...
* $Y = \beta_0 + \beta_1* X +\epsilon,$ where $\epsilon~\sim N(0,\sigma)$ is a *statistical* (equiv. *stochastic*, *random*) linear model ($\sim$ means '*is drawn from*')
...
* *Challenge* What have we plotted? (tip: google, e.g., 'probability distribution')
